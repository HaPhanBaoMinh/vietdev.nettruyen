...
class Image(models.Model):
    def rename_image(instance, filename):
        ...
        # Định dạng lại tên tệp ảnh thành 'YYYYMMDDHHMMSS.jpg'
        filename = current_time.strftime('%Y%m%d%H%M%S') + '.jpg'
        # Trả về tên tệp ảnh mới
        return os.path.join('chap/', filename)

    image = models.ImageField(upload_to=rename_image, null=True)
    chap = models.ForeignKey(
        Chap, on_delete=models.CASCADE, related_name="chap_image")
    order = models.IntegerField(unique=True)
    comic = models.ForeignKey(
        Comic, on_delete=models.CASCADE, related_name="comic", default=None)

    class Meta:
        # Sắp xếp các bản ghi theo trường 'order'
        ordering = ['order']
